...
if mode == "create":
    ...
    with db.get_session() as session:
        ...
        for level in range(band.levels + 1):
            ...
            tile_array = np.empty((n_tiles_x * band.tile_size, n_tiles_y * band.tile_size), order="C")
            ...
            for tile in tiles:
                ...
                x = tile.x
                y = tile.y
                tile_data = np.frombuffer(tile.data, dtype=np.float64).reshape(band.tile_size, band.tile_size)
