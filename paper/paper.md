...
**Web Mercator static tiling**, the same tiling scheme used by consumer mapping services such as Google Maps, is by far the most common standard for browser-based astronomical image viewing. Because so many tools share this standard, it offers strong interoperability: a new dataset can typically be added to an existing Web Mercator viewer with minimal effort. Many major sky surveys provide viewers or public tile sets that follow this convention, or a close variant of it, including unWISE, the Legacy Survey, LSST (preview), Euclid (preview), GALEX, SDSS, VLASS, and Pan-STARRS; some depart from the strict Web Mercator projection in favor of a custom one while retaining the same static-tile approach. The standard's principal limitation is that, because each tile is a pre-rendered image, the display cannot be meaningfully adjusted after the fact. For example, the colormap or intensity scaling cannot be changed once a tile has been generated.
...
**The NASA LAMBDA ACT Viewer** [@actviewer] is a purpose-built web viewer for the ACT DR5 data release, hosted at the NASA Legacy Archive for Microwave Background Data Analysis. It provides a tile-based, Google Maps-style interface for ACT maps and supports keyboard-driven switching between frequency bands (f090, f150, f220), Stokes components (T, Q, U, E, B), and data releases. It also supports region selection and FITS cutout download. Its tiling is a variant of the Web Mercator static-tile approach described above: rather than pre-rendering a final display image, the raw FITS pixel values are themselves encoded into the tiles' PNG data, and the browser reconstructs the displayed image from that encoded data with JavaScript at render time — which is what allows features such as colormap adjustment despite the underlying tiles being static images. The viewer is configured through a JavaScript file that declares each map, frequency band, and Stokes component as its own tile layer with an associated colormap and scaling. However, adding a new dataset still requires a separate step to pre-encode its FITS data into this tile format, rather than serving tiles on-the-fly directly from the original FITS files as `TileMaker` does. It also does not expose a general API or a built-in mechanism for overlaying custom source catalogs or highlight regions. Keyboard-only navigation, while functional, is less discoverable than a graphical layer menu for users exploring an unfamiliar dataset with many bands and layers.
...
Each zoom level of a layer's tile pyramid is generated directly from the original FITS file at request time: coarser zoom levels are produced by subsampling the native-resolution pixel grid rather than by averaging blocks of pixels together. Most tile-based viewers implement a pre-computed, block-averaged approach as a means to reduce per-pixel noise as a user zooms out. `TileMaker`'s subsampling approach avoids the extra storage and pre-computation a block-averaged pyramid would require, consistent with its zero-pre-ingestion, no-ancillary-files design (see Statement of need). The trade-off is that coarse zoom levels do not benefit from the noise reduction block-averaging provides.

`TileViewer` is a single-page application built with TypeScript + React. React is mature, well documented, and remains a popular library for single-page applications. As such, using React ensures that there is a large community of developers capable of contributing to and maintaining the UI. TypeScript further improves the UI's maintainability by clearly defining expected types for server responses, React components, and data structures. In choosing between available map libraries, the two main contenders were OpenLayers and LeafletJS. Both are lightweight, versatile, open source, and widely used. Ultimately, OpenLayers was selected for two reasons. One, OpenLayers seamlessly handles arbitrary tile size differences between layers. While Leaflet does provide built-in support for varying tile sizes, Leaflet expects differences to be powers of two (e.g. 256 vs. 512 pixels); tile sizes in SO maps do not conform to this constraint, making OpenLayers the more suitable choice. Two, OpenLayers makes it easy to reproject external tilesets that use a different projection than SO's own, allowing any tileset to be overlaid onto SO data. `TileMaker` is built with Python + FastAPI and its [PyPI package](https://pypi.org/project/tilemaker/) includes the static, bundled `TileViewer` to make it a standalone, out-of-the-box web application. FastAPI was chosen for its ease of use, speed, and documentation.
