...
class TileCache(abc.ABC):
    @abc.abstractmethod
    def get_cache(self, band:int, x:int, y:int, level:int, proprietary: bool) -> np.ndarray | None:
        raise NotImplementedError
    
    @abc.abstractmethod
    def set_cache(self, band:int, x:int, y:int, level:int, data: np.ndarray | None, proprietary: bool) -> None:
        raise NotImplementedError
...
class PassThroughCache(TileCache):
    def get_cache(self, band:int, x:int, y:int, level:int, proprietary: bool) -> np.ndarray | None:
        ...
        """
        raise TileNotFound(
            f"Tile {band}-{level}-{x}-{y} not found in cache."
        )
